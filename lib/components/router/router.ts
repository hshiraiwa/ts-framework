...
export interface RouteMap {
  get?: { [path: string]: string | Route | RouteDefs };
  post?: { [path: string]: string | Route | RouteDefs };
  put?: { [path: string]: string | Route | RouteDefs };
  delete?: { [path: string]: string | Route | RouteDefs };
}
...
export default class ServerRouter {
   ...
   */
  static build(controllers: object | string, routes: RouteMap | string, options?: RouterOptions) {
    let wrapper;

    if (routes && util.isString(routes)) {
      wrapper = new ServerRouter(controllers, require(routes), options);
    } else {
      wrapper = new ServerRouter(controllers, routes as RouteMap, options);
    }
